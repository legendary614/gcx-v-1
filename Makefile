...
pkgdatadir = $(datadir)/gcx
pkgincludedir = $(includedir)/gcx
pkglibdir = $(libdir)/gcx
pkglibexecdir = $(libexecdir)/gcx
...
CONFIG_HEADER = $(top_builddir)/src/config/gcx-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/gcx-config.h.in \
...
ACLOCAL = ${SHELL} /root/working/gcx-1/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/working/gcx-1/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/working/gcx-1/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/working/gcx-1/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = gcx-cli
BITCOIN_DAEMON_NAME = gcxd
BITCOIN_GUI_NAME = gcx-qt
BITCOIN_TX_NAME = gcx-tx
...
MAINT = 
MAKEINFO = ${SHELL} /root/working/gcx-1/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = gcx
PACKAGE_BUGREPORT = gcx.io
PACKAGE_NAME = GCX Core
PACKAGE_STRING = GCX Core 1.0.0
PACKAGE_TARNAME = gcx
PACKAGE_URL = 
...
abs_builddir = /root/working/gcx-1
abs_srcdir = /root/working/gcx-1
abs_top_builddir = /root/working/gcx-1
abs_top_srcdir = /root/working/gcx-1
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/working/gcx-1/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/gcxd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/gcx-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/gcx-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_gcx_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_gcx_coverage.info test_gcx.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/GCX-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/gcx-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/gcx-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/gcx-config.h
$(top_srcdir)/src/config/gcx-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/gcx-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/GCX-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GCX-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GCX-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_gcx.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_gcx -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_gcx_filtered.info: test_gcx.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_gcx_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_gcx_coverage.info: baseline_filtered_combined.info test_gcx_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_gcx_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_gcx_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_gcx_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_gcx.coverage/.dirstamp:  test_gcx_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_gcx.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local:
	rm -rf test_gcx.coverage/ total.coverage/ $(OSX_APP)
